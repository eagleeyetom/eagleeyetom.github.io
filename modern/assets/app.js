...
function canUseLocalStorage() {
    try {
        ...
        console.warn('Local storage unavailable; settings will not persist.', error);
        return false;
    }
}
...
function updateContrastLabel(element, isHighContrast) {
    if (element) {
        element.setAttribute('aria-label', isHighContrast ? element.dataset.labelOn ?? 'High contrast on' : element.dataset.labelOff ?? 'High contrast off');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const increaseButton = document.querySelector('[data-action="font-increase"]');
    const decreaseButton = document.querySelector('[data-action="font-decrease"]');
    ...
    function applyFontScale(scale) {
        ...
        root.style.setProperty('--base-font-size', `${DEFAULT_FONT_SIZE * clamped}px`);
    ...
    if (cookieNotice && acceptCookiesButton) {
        ...
        try {
            const consent = storageAvailable ? window.localStorage.getItem(cookieKey) : null;
            if (consent === 'accepted') {
    ...
    if (storedLanguage && storedLanguage !== defaultLanguage && body.dataset.redirectToPreferredLanguage === 'true') {
        const currentPath = window.location.pathname;
        const preferredPath = currentPath.replace(/\/(en|pl)\//, `/${storedLanguage}/`);
